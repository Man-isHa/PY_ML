...
def get_data():
	...
	with open('./data/overlapped/class1_train.txt', 'r') as f:
    		lines = f.readlines()
	...
	with open('./data/overlapped/class2_train.txt', 'r') as f:
    		lines = f.readlines()
	...
	with open('./data/overlapped/class1_val.txt', 'r') as f:
    		lines = f.readlines()
	...
	with open('./data/overlapped/class2_val.txt', 'r') as f:
    		lines = f.readlines()
	...
	with open('./data/overlapped/class1_test.txt', 'r') as f:
    		lines = f.readlines()
	...
	with open('./data/overlapped/class2_test.txt', 'r') as f:
    		lines = f.readlines()
...
def build_model():
	...
	X = tf.placeholder("float", shape=[None, 2])
    	...
    	# Weight initializations(4 hidden layers)
	'''
	...
	W_2 = tf.Variable(tf.random_normal([h1_size, h2_size]), name='weights')
   	b_2 = tf.Variable(tf.zeros([h2_size], name='bias'))
    	W_3 = tf.Variable(tf.random_normal([h2_size, h3_size]), name='weights')
   	b_3 = tf.Variable(tf.zeros([h3_size], name='bias'))
	W_4 = tf.Variable(tf.random_normal([h3_size, NLABELS]), name='weights')
   	b_4 = tf.Variable(tf.zeros([NLABELS], name='bias'))
	

	
	# Weight initializations(2 hidden layers)
	W_1 = tf.Variable(tf.random_normal([2, h1_size]), name='weights')
   	b_1 = tf.Variable(tf.zeros([h1_size], name='bias'))
	W_2 = tf.Variable(tf.random_normal([h1_size, h2_size]), name='weights')
   	b_2 = tf.Variable(tf.zeros([h2_size], name='bias'))
	...
	'''	
	# Weight initializations(1 hidden layers)
	W_1 = tf.Variable(tf.random_normal([2, h1_size]), name='weights')
   	b_1 = tf.Variable(tf.zeros([h1_size], name='bias'))
	...
	# Forward propagation
	#yhat = tf.nn.sigmoid(tf.matmul(tf.nn.sigmoid(tf.matmul(tf.nn.sigmoid(tf.matmul(tf.nn.sigmoid(tf.matmul(X, W_1) + b_1), W_2) + b_2), W_3) + b_3), W_4) + b_4)
	h1 = tf.nn.sigmoid(tf.matmul(X, W_1) + b_1)
	yhat = tf.nn.sigmoid(tf.matmul(h1, W_2) + b_2)
    	#yhat = tf.nn.sigmoid(tf.matmul(tf.nn.sigmoid(tf.matmul(tf.nn.sigmoid(tf.matmul(X, W_1) + b_1), W_2) + b_2), W_3) + b_3)
	#yhat = tf.nn.sigmoid(tf.matmul(tf.nn.sigmoid(tf.matmul(X, W_1) + b_1), W_2) + b_2)
   	predict = tf.argmax(yhat, dimension=1)

    	# Backward propagation
    	cost    = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(yhat, y))
    	updates = tf.train.RMSPropOptimizer(0.0001,decay = 0.9).minimize(cost)
	return X, y, predict, updates, h1, yhat

def main():
	checkpoint_file = './hello3.chk'
	...
	test_y = test[:,2:4]
    	...
    	# Run SGD
	X, y, predict, updates, h1, yhat = build_model()
    	sess = tf.Session()
